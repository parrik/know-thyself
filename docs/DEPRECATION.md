...
5. **The durable thing is the discipline, not the schema.** If frontier models do the typing, then what this scaffold actually scaffolds is the practice of refusing to let them write the rules you live by. The hard rule that a practice waits for an explicit human yes while the rest of the record fills in autonomously (see SCHEMA.md's write gate). The provenance chain a human can stand behind in front of another human. The willingness to mark a claim *open* and leave it open. Discipline doesn't get inferred from messy notes — it gets practiced. Build for the practice; the typed structure is the trace of the practice, not the product.
